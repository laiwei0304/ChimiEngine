...
protected:
    ...
    void OnInit() override {
        ...
        mDescriptorSetLayout = std::make_shared<chimi::CmVKDescriptorSetLayout>(device, desctLayoutBindings);

        chimi::ShaderLayout shaderLayout = {
                .descriptorSetLayouts = { mDescriptorSetLayout->GetHandle() }
        };
        mPipelineLayout = std::make_shared<chimi::CmVKPipelineLayout>(device,
                                                                    CHIMI_SHADER_OUTPUT_DIR"/02_descriptor_set.vert",
                                                                    CHIMI_SHADER_OUTPUT_DIR"/02_descriptor_set.frag",
                                                                    shaderLayout);
        std::vector<VkVertexInputBindingDescription> vertexBindings = {
            {
                .binding = 0,
                .stride = sizeof(chimi::CmVertex),
        ...
        std::vector<VkVertexInputAttributeDescription> vertexAttrs = {
            {
                ...
                .offset = offsetof(chimi::CmVertex, position)
            },
            {
                .location = 1,
                .binding = 0,
                .format = VK_FORMAT_R32G32_SFLOAT,
                .offset = offsetof(chimi::CmVertex, texcoord0)
            },
            {
                .location = 2,
                .binding = 0,
                .format = VK_FORMAT_R32G32B32_SFLOAT,
                .offset = offsetof(chimi::CmVertex, normal)
            }
        };
        mPipeline = std::make_shared<chimi::CmVKPipeline>(device, mRenderPass.get(), mPipelineLayout.get());
        ...
        mDescriptorPool = std::make_shared<chimi::CmVKDescriptorPool>(device, 1, poolSizes);
        mDescriptorSets = mDescriptorPool->AllocateDescriptorSet(mDescriptorSetLayout.get(), 1);

        mGlobalBuffer = std::make_shared<chimi::CmVKBuffer>(device, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, sizeof(GlobalUbo), nullptr, true);
        mInstanceBuffer = std::make_shared<chimi::CmVKBuffer>(device, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, sizeof(InstanceUbo), nullptr, true);
        mSampler = std::make_shared<chimi::CmSampler>();
        mTexture0 = std::make_shared<chimi::CmTexture>(CHIMI_RES_TEXTURE_DIR"Checkerboard.png");
        mTexture1 = std::make_shared<chimi::CmTexture>(CHIMI_RES_TEXTURE_DIR"R-C.jpeg");
    ...
    void OnRender() override {
        ...
        chimi::CmVKFrameBuffer *frameBuffer = mRenderTarget->GetFrameBuffer();
...
private:
    ...
    std::shared_ptr<chimi::CmVKDescriptorSetLayout> mDescriptorSetLayout;
    std::shared_ptr<chimi::CmVKDescriptorPool> mDescriptorPool;
    std::vector<VkDescriptorSet> mDescriptorSets;
    std::shared_ptr<chimi::CmVKPipelineLayout> mPipelineLayout;
    std::shared_ptr<chimi::CmVKPipeline> mPipeline;

    std::shared_ptr<chimi::CmMesh> mCubeMesh;
    GlobalUbo mGlobalUbo;
    InstanceUbo mInstanceUbo;
    std::shared_ptr<chimi::CmVKBuffer> mGlobalBuffer;
    std::shared_ptr<chimi::CmVKBuffer> mInstanceBuffer;
    std::shared_ptr<chimi::CmTexture> mTexture0;
    std::shared_ptr<chimi::CmTexture> mTexture1;
    std::shared_ptr<chimi::CmSampler> mSampler;
