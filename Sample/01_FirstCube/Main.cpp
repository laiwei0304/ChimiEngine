...
int main(){
    ...
    std::unique_ptr<chimi::CmWindow> window = chimi::CmWindow::Create(800, 600, "01_FirstCube");
    ...
    std::shared_ptr<chimi::CmVKRenderPass> renderPass = std::make_shared<chimi::CmVKRenderPass>(device.get(), attachments, subpasses);
    ...
    for(int i = 0; i < swapchainImageSize; i++){
        std::vector<std::shared_ptr<chimi::CmVKImage>> images = {
            ...
            std::make_shared<chimi::CmVKImage>(device.get(), imageExtent, depthFormat, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT)
        };
    ...
    std::shared_ptr<chimi::CmVKPipelineLayout> pipelineLayout = std::make_shared<chimi::CmVKPipelineLayout>(device.get(),
                                                                                                        CHIMI_SHADER_OUTPUT_DIR"/01_hello_buffer.vert",
                                                                                                        CHIMI_SHADER_OUTPUT_DIR"/01_hello_buffer.frag", shaderLayout);
    ...
    std::shared_ptr<chimi::CmVKBuffer> vertexBuffer = std::make_shared<chimi::CmVKBuffer>(device.get(), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                                                                                      sizeof(vertices[0]) * vertices.size(), vertices.data());
    std::shared_ptr<chimi::CmVKBuffer> indexBuffer = std::make_shared<chimi::CmVKBuffer>(device.get(), VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
