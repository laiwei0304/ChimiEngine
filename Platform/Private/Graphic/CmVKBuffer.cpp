...
namespace chimi{
    CmVKBuffer::CmVKBuffer(CmVKDevice *device, VkBufferUsageFlags usage, size_t size, void *data, bool bHostVisible)
                    : mDevice(device), mSize(size), bHostVisible(bHostVisible)  {
        CreateBuffer(usage, data);
    }

    CmVKBuffer::~CmVKBuffer() {
        VK_D(Buffer, mDevice->GetHandle(), mHandle);
        VK_F(mDevice->GetHandle(), mMemory);
    }

    void CmVKBuffer::CreateBuffer(VkBufferUsageFlags usage, void *data) {
        if(bHostVisible){
    ...
    void CmVKBuffer::CreateBufferInternal(CmVKDevice *device, VkMemoryPropertyFlags memProps, VkBufferUsageFlags usage, size_t size, VkBuffer *outBuffer, VkDeviceMemory *outMemory) {
    ...
    void CmVKBuffer::CopyToBuffer(CmVKDevice *device, VkBuffer srcBuffer, VkBuffer dstBuffer, size_t size) {
    ...
    VkResult CmVKBuffer::WriteData(void *data) {
