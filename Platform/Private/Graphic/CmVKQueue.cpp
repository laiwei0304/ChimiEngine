...
namespace chimi
{
    CmVKQueue::CmVKQueue(uint32_t familyIndex, uint32_t index, VkQueue queue, bool canPresent)
    ...
    void CmVKQueue::WaitIdle() const {
        CALL_VK(vkQueueWaitIdle(mHandle));
    }

    void CmVKQueue::Submit(const std::vector<VkCommandBuffer> &cmdBuffers, const std::vector<VkSemaphore> &waitSemaphores, const std::vector<VkSemaphore> &signalSemaphores, VkFence frameFence) {
