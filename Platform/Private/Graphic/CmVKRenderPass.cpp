...
namespace chimi
{
    CmVKRenderPass::CmVKRenderPass(CmVKDevice *device, const std::vector<Attachment> &attachments, const std::vector<RenderSubPass> &subPasses)
    ...
    CmVKRenderPass::~CmVKRenderPass() {
        VK_D(RenderPass, mDevice->GetHandle(), mHandle);
    }

    void CmVKRenderPass::Begin(VkCommandBuffer cmdBuffer, CmVKFrameBuffer *frameBuffer, const std::vector<VkClearValue> &clearValues) const {
        VkRect2D renderArea = {
                .offset = { 0, 0 },
    ...
    void CmVKRenderPass::End(VkCommandBuffer cmdBuffer) const {
