...
namespace chimi{
...
#ifdef CHIMI_ENGINE_PLATFORM_WIN32
#elif CHIMI_ENGINE_PLATFORM_MACOS
            { "VK_KHR_portability_subset", true },
#elif CHIMI_ENGINE_PLATFORM_LINUX
#endif
    };

    CmVKDevice::CmVKDevice(CmVKGraphicContext *context, uint32_t graphicQueueCount, uint32_t presentQueueCount, const CmVkSettings &settings) : mContext(context), mSettings(settings) {
        if(!context){
        ...
        for(int i = 0; i < graphicQueueCount; i++){
            ...
            mGraphicQueues.push_back(std::make_shared<CmVKQueue>(graphicQueueFamilyInfo.queueFamilyIndex, i, queue, false));
        }
        for(int i = 0; i < presentQueueCount; i++){
            VkQueue queue;
            vkGetDeviceQueue(mHandle, presentQueueFamilyInfo.queueFamilyIndex, i, &queue);
            mPresentQueues.push_back(std::make_shared<CmVKQueue>(presentQueueFamilyInfo.queueFamilyIndex, i, queue, true));
        }
    ...
    void CmVKDevice::CreatePipelineCache() {
    ...
    void CmVKDevice::CreateDefaultCmdPool() {
        mDefaultCmdPool = std::make_shared<chimi::CmVKCommandPool>(this, mContext->GetGraphicQueueFamilyInfo().queueFamilyIndex);
    }

    int32_t CmVKDevice::GetMemoryIndex(VkMemoryPropertyFlags memProps, uint32_t memoryTypeBits) const {
    ...
    VkCommandBuffer CmVKDevice::CreateAndBeginOneCmdBuffer() {
        VkCommandBuffer cmdBuffer = mDefaultCmdPool->AllocateOneCommandBuffer();
        mDefaultCmdPool->BeginCommandBuffer(cmdBuffer);
        return cmdBuffer;
    }

    void CmVKDevice::SubmitOneCmdBuffer(VkCommandBuffer cmdBuffer) {
        mDefaultCmdPool->EndCommandBuffer(cmdBuffer);
        CmVKQueue *queue = GetFirstGraphicQueue();
        queue->Submit({ cmdBuffer });
        queue->WaitIdle();
    }

    VkResult CmVKDevice::CreateSimpleSampler(VkFilter filter, VkSamplerAddressMode addressMode, VkSampler *outSampler) {
