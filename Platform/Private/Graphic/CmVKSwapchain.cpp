...
namespace chimi
{
    static VkCompositeAlphaFlagBitsKHR ChooseCompositeAlpha(const VkSurfaceCapabilitiesKHR& capabilities) {
        ...
        for (VkCompositeAlphaFlagBitsKHR mode : preferredModes) {
    ...
    CmVKSwapchain::CmVKSwapchain(CmVKGraphicContext *context, CmVKDevice *device) : mContext(context), mDevice(device) {
        ReCreate();
    }

    CmVKSwapchain::~CmVKSwapchain() {
        VK_D(SwapchainKHR, mDevice->GetHandle(), mHandle);
    }

    bool CmVKSwapchain::ReCreate() {
        LOG_D("-----------------------------");
        ...
        VkSwapchainCreateInfoKHR swapchainInfo = {
                ...
                .compositeAlpha = ChooseCompositeAlpha(mSurfaceInfo.capabilities),
    ...
    VkResult CmVKSwapchain::AcquireImage(int32_t *outImageIndex, VkSemaphore semaphore, VkFence fence) {
        uint32_t imageIndex;
    ...
    VkResult CmVKSwapchain::Present(int32_t imageIndex, const std::vector<VkSemaphore> &waitSemaphores) {
