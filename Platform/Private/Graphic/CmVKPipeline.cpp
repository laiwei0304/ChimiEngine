...
namespace chimi
{
    CmVKPipelineLayout::CmVKPipelineLayout(CmVKDevice *device, const std::string &vertexShaderFile, const std::string &fragShaderFile, const ShaderLayout &shaderLayout) : mDevice(device) {
        // compile shaders
    ...
    CmVKPipelineLayout::~CmVKPipelineLayout() {
    ...
    VkResult CmVKPipelineLayout::CreateShaderModule(const std::string &filePath, VkShaderModule *outShaderModule) {
    ...
    CmVKPipeline::CmVKPipeline(CmVKDevice *device, CmVKRenderPass *renderPass, CmVKPipelineLayout *pipelineLayout) : mDevice(device), mRenderPass(renderPass), mPipelineLayout(pipelineLayout) {

    }

    CmVKPipeline::~CmVKPipeline() {
        VK_D(Pipeline, mDevice->GetHandle(), mHandle);
    }

    void CmVKPipeline::Create() {
    ...
    CmVKPipeline *CmVKPipeline::SetVertexInputState(const std::vector<VkVertexInputBindingDescription> &vertexBindings,
    ...
    CmVKPipeline *CmVKPipeline::SetInputAssemblyState(VkPrimitiveTopology topology, VkBool32 primitiveRestartEnable) {
    ...
    CmVKPipeline *CmVKPipeline::SetRasterizationState(const PipelineRasterizationState &rasterizationState) {
    ...
    CmVKPipeline *CmVKPipeline::SetMultisampleState(VkSampleCountFlagBits samples, VkBool32 sampleShadingEnable, float minSampleShading) {
    ...
    CmVKPipeline *CmVKPipeline::SetDepthStencilState(const PipelineDepthStencilState &depthStencilState) {
    ...
    CmVKPipeline *CmVKPipeline::SetColorBlendAttachmentState(VkBool32 blendEnable, VkBlendFactor srcColorBlendFactor,
    ...
    CmVKPipeline *CmVKPipeline::SetDynamicState(const std::vector<VkDynamicState> &dynamicStates) {
        mPipelineConfig.dynamicState.dynamicStates = dynamicStates;
        return this;
    }

    CmVKPipeline *CmVKPipeline::EnableAlphaBlend() {
    ...
    CmVKPipeline *CmVKPipeline::EnableDepthTest() {
    ...
    void CmVKPipeline::Bind(VkCommandBuffer cmdBuffer) {
