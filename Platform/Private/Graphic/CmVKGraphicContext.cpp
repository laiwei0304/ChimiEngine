...
namespace chimi
{
    ...
    CmVKGraphicContext::~CmVKGraphicContext() {
    ...
    void CmVKGraphicContext::CreateSurface(CmWindow *window) {
        if(!window){
            LOG_E("window is not exists.");
            return;
        }

        auto *glfWwindow = dynamic_cast<CmGLFWwindow *>(window);
        if(!glfWwindow){
            // FIXME
    ...
    void CmVKGraphicContext::SelectPhyDevice() {
    ...
    void CmVKGraphicContext::PrintPhyDeviceInfo(VkPhysicalDeviceProperties &props) {
    ...
    uint32_t CmVKGraphicContext::GetPhyDeviceScore(VkPhysicalDeviceProperties &props) {
