...
enum Key{
    ...
    enum KeyMod{
        CHIMI_MOD_SHIFT              = 0x0001,
        CHIMI_MOD_CONTROL            = 0x0002,
        CHIMI_MOD_ALT                = 0x0004,
        CHIMI_MOD_SUPER              = 0x0008,
        CHIMI_MOD_CAPS_LOCK          = 0x0010,
        CHIMI_MOD_NUM_LOCK           = 0x0020,
    };

    static const char* CmKeyToStr(Key keyCode){
        switch (keyCode) {
    ...
    static const char* CmKeyModToStr(KeyMod keyMod){
        switch (keyMod) {
            ENUM_TO_STR(CHIMI_MOD_SHIFT);
            ENUM_TO_STR(CHIMI_MOD_CONTROL);
            ENUM_TO_STR(CHIMI_MOD_ALT);
            ENUM_TO_STR(CHIMI_MOD_SUPER);
            ENUM_TO_STR(CHIMI_MOD_CAPS_LOCK);
            ENUM_TO_STR(CHIMI_MOD_NUM_LOCK);
        }
        return "unknown";
    }

    class CmKeyPressEvent : public CmEvent{
    public:
        CmKeyPressEvent(Key keyCode, KeyMod keyMod, bool repeat) : mKeyCode(keyCode), mKeyMod(keyMod), mRepeat(repeat){
        ...
        [[nodiscard]] std::string ToString() const override {
            std::stringstream ss;
            ss << CmEvent::ToString();
            ss << "( keyCode=" << CmKeyToStr(mKeyCode);
        ...
        [[nodiscard]] bool IsCapsLockPressed() const { return mKeyMod & CHIMI_MOD_CAPS_LOCK; };
        [[nodiscard]] bool IsNumLockPressed() const { return mKeyMod & CHIMI_MOD_NUM_LOCK; };
    ...
    class CmKeyReleaseEvent : public CmEvent{
    public:
        CmKeyReleaseEvent(Key keyCode) : mKeyCode(keyCode){
